...
# Generates components for Manuscript Figure 5
# ==============================================================================
...
p_oob <- ggplot(oob_long, aes(x = Trees, y = Error_Rate, color = Error_Type)) + geom_line(linewidth = 1.2, alpha = 0.9) + scale_color_manual(values = c("OOB" = "#000000", "ALS" = "#E64B35", "Control" = "#4DBBD5"), name = "Error Type") + scale_y_continuous(labels = percent_format(accuracy = 1), limits = c(0, 0.45), expand = c(0, 0)) + scale_x_continuous(expand = c(0, 0)) + labs(title = "5A. Random Forest Out-of-Bag Error Rate", x = "Number of Trees", y = "Error Rate") + theme_publication() + theme(legend.position = "right", legend.justification = "center", legend.background = element_rect(fill = "white", color = NA), legend.key = element_rect(fill = "white"), legend.key.size = unit(0.9, "lines"), legend.spacing.y = unit(0.15, "cm"), legend.margin = ggplot2::margin(5, 5, 5, 5, "pt"))
...
roc_data <- data.frame(
  ...
  CI_Lower = as.numeric(ci_shape[, 1]), 
  CI_Upper = as.numeric(ci_shape[, 3])  
)
...
p_roc <- ggplot(roc_data, aes(x = 1 - Specificity, y = Sensitivity)) +
  ...
  coord_fixed() +
  labs(title = "5F. Consensus Gene Signature ROC Curve", x = "1 - Specificity (False Positive Rate)", y = "Sensitivity (True Positive Rate)") +
  theme_publication() + theme(panel.grid.major = element_line(color = "gray90", linewidth = 0.3), aspect.ratio = 1)

ggsave("Manuscript_Figures/Step5_MachineLearning/Figure5F_Consensus_ROC.pdf", plot = p_roc, width = 7, height = 7)
ggsave("Manuscript_Figures/Step5_MachineLearning/Figure5F_Consensus_ROC.png", plot = p_roc, width = 7, height = 7, dpi = 600)

# G. VENN DIAGRAM (Figure 5G)
p_venn <- ggVennDiagram(gene_lists, label = "count", label_alpha = 0, edge_size = 1.5, set_size = 5) + scale_fill_gradient(low = "#FFF5EB", high = "#E64B35", name = "Gene Count") + scale_color_manual(values = c("#4DBBD5", "#E64B35", "#00A087")) + labs(title = paste0("5G. Triple-Algorithm Consensus (n = ", length(consensus_genes), " genes)")) + theme_void(base_size = 14) + theme(plot.title = element_text(face = "bold", size = 16, hjust = 0.5), legend.position = "right", legend.title = element_text(face = "bold"))
ggsave("Manuscript_Figures/Step5_MachineLearning/Figure5G_Venn.pdf", plot = p_venn, width = 8, height = 6)
ggsave("Manuscript_Figures/Step5_MachineLearning/Figure5G_Venn.png", plot = p_venn, width = 8, height = 6, dpi = 600)

message("Step 5 Complete. Figure 5 generated.")
